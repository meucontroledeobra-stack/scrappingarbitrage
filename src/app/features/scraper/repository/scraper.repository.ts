    ...
    const coins = await page.$$eval(
      ...
      (trs) =>
        trs.map((tr) => {
          ...
          const coinName = (tds[0] as HTMLElement)?.innerText.trim();
          const fundingCells = tds.slice(1).map((td) => {
            const a = td.querySelector("a") as HTMLAnchorElement | null;
            return a
    ...
    > = {
          ...
          return page.evaluate(() => {
            const preco =
              document.querySelector<HTMLSpanElement>(
                "h2.contractDetail_lastPrice__bKyQo span"
              )?.innerText || null;
      ...
      "bybit.com": async (page) => {
        ...
        return page.evaluate(() => {
          ...
          let taxa: string | null = rateEl?.textContent?.trim() || null;
          let horario: string | null = null;
          const timerText =
          ...
          const processOrders = (selector: string) => {
      ...
      "kucoin.com": async (page) => {
        ...
        return page.evaluate(() => {
          ...
          const parseOrders = (selector: string, priceClass: string) => {
            const orders: {
              preco: string;
              quantidade: string;
              acumulado: string;
      ...
      "lbank.com": async (page) => {
        ...
        return page.evaluate(() => {
          ...
          const parseOrders = (selector: string) => {
            const orders: {
              preco: string;
              quantidade: string;
              acumulado: string;
      ...
      "derivatives.bitmart.com": async (page) => {
        ...
        return page.evaluate(() => {
          ...
          const parseOrders = (selector: string, type: "buy" | "sell") => {
            const orders: {
              preco: string;
              quantidade: string;
              acumulado: string;
      ...
      "coinex.com": async (page) => {
        ...
        return page.evaluate(() => {
          ...
          const fundingFlex = document.querySelector<HTMLDivElement>(
            ".via-ticker-item.flex"
          );
          if (fundingFlex) {
            const leftRate =
          ...
          const asks = Array.from(
            document.querySelectorAll<HTMLDivElement>(".list-item.sell")
          ).map((el) => {
            const preco =
          ...
          const bids = Array.from(
            document.querySelectorAll<HTMLDivElement>(".list-item.buy")
          ).map((el) => {
            const preco =
              txt(el.querySelector(".td-item:nth-child(1) span span")) || "0";
            const quantidade =
      ...
      for (const cell of coin.fundingCells) {
        if (typeof cell === "object" && "link" in cell) {
          ...
          try {
            ...
            if (!extractor) {
              console.log(
                `${coin.coinName} sem configuração de seletores para ${cell.link}`
              );
            } else {
