...
async function scrapeFundingRates() {
    ...
    await page.setUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.188 Safari/537.36");
    ...
    const coins = await page.$$eval("._tableContainer_1vuyk_14 table tbody tr", (trs) => trs.map((tr) => {
    ...
    const selectors = {
        ...
        "gate.io": async (page) => {
            ...
            await page.waitForFunction(() => {
                const timerDiv = document.querySelector('[data-testid="futures-header-funding-rate-timer"]');
                if (!timerDiv)
                    return false;
                const span = timerDiv.querySelector("span.font-add-color, span.font-dec-color");
                return span && span.textContent && span.textContent.trim() !== "";
            }, { timeout: 20000 });
            return page.evaluate(() => {
                const preco = document.querySelector('[data-testid="tr-futures-header-price"] span')?.innerText || null;
                let taxa = null;
                let horario = null;
                const timerDiv = document.querySelector('[data-testid="futures-header-funding-rate-timer"]');
                if (timerDiv) {
                ...
                document.querySelectorAll(".orderbook-item").forEach((row) => {
                    const preco = (row.querySelector("div.text-decrease, div.text-increase")
                        ?.textContent || "").trim();
        ...
        "mexc.com": async (page) => {
            ...
            try {
                ...
                return page.evaluate(() => {
                    const preco = document.querySelector("h2.contractDetail_lastPrice__bKyQo span")?.innerText || null;
                    const taxaHorarioRaw = document.querySelector(".contractDetail_fundingItem__TbJSH i.font-16")?.innerText || "";
                    let taxa = null;
                    let horario = null;
                    ...
                    // Bids
                    const bids = Array.from(document.querySelectorAll(".market_bidRow__6wAE6")).map((row) => ({
                        preco: row
                            .querySelector(".market_price__V_09X.market_buy__F9O7S span")
                            ?.textContent?.trim() || "",
                        quantidade: row.querySelector(".market_vol__M6Ton")?.textContent?.trim() ||
                            "",
                        acumulado: row.querySelector(".market_amount__a1I8g")?.textContent?.trim() ||
                            "",
                    }));
                    // Asks
                    const asks = Array.from(document.querySelectorAll(".market_askRow__eRJes")).map((row) => ({
                        preco: row
        ...
        "bitget.com": async (page) => {
            ...
            await page.waitForFunction(() => {
                const priceEl = document.querySelector('[data-testid="FutureOrderTableCurrentPrice"]');
                return (priceEl &&
                    priceEl.textContent &&
                    priceEl.textContent.trim() !== "0.0000");
            }, { timeout: 20000 });
            return page.evaluate(() => {
                const precoEl = document.querySelector('[data-testid="FutureOrderTableCurrentPrice"]');
                const preco = precoEl?.innerText.trim() || null;
                const taxaEl = document.querySelector('[data-testid="FutureCurrentContractFundsRate"]');
                const horarioEl = document.querySelector('[data-testid="FutureCurrentContractFundsRateCountDown"]');
        ...
        "bybit.com": async (page) => {
            ...
            return page.evaluate(() => {
                ...
                const rateEl = document.querySelector("div.pnr__item.pnr__funding-rate-fixed span.pnr__brand");
                let taxa = rateEl?.textContent?.trim() || null;
                let horario = null;
                const timerText = rateEl?.parentElement?.nextElementSibling?.textContent || "";
                ...
                const processOrders = (selector) => {
                    ...
                    let acumulado = 0;
                    return rows.map((row) => {
                        const preco = row.querySelector(".ob__table-price div")?.textContent?.trim() ||
                            "";
                        const quantidade = row.querySelector(".ob__table-qty")?.textContent?.trim() || "";
        ...
        "kucoin.com": async (page) => {
            ...
            return page.evaluate(() => {
                ...
                const items = Array.from(document.querySelectorAll("div.hover_K1heJ"));
                items.forEach((item) => {
                    const title = item
                    ...
                    switch (title) {
                        ...
                        case "Funding Settlement":
                            horario =
                                item
                                    .querySelector("div.value_R8fIA span.ant-statistic-content-value")
                                    ?.textContent?.trim() || null;
                            break;
                    }
                });
                ...
                const parseOrders = (selector, priceClass) => {
                    const orders = [];
                    let acumulado = 0;
                    const rows = document.querySelectorAll(`${selector} .orderRow_DYAUi`);
                    rows.forEach((row) => {
                        const preco = row.querySelector(`.${priceClass}`)?.textContent?.trim() || "";
                        const quantidadeStr = row.querySelector(".size_hAIQj")?.textContent?.trim() || "0";
                        let quantidade = parseFloat(quantidadeStr.replace(/K/, "000").replace(/,/g, "")) ||
                            0;
                ...
                const asks = parseOrders(".ku-orderBook.book_XWMLV:first-of-type", "sell_gyttN");
                const bids = parseOrders(".ku-orderBook.book_XWMLV:last-of-type", "buy_tfgD8");
                return {
                    preco: preco || "0",
                    taxa,
                    horario,
            ...
            return page.evaluate(() => {
                ...
                for (const item of items) {
                    ...
                    if (label.includes("Funding") || label.includes("Financiamento")) {
                        ...
                        const countdown = item
                            ...
                            ?.textContent?.trim() || null;
                        if (countdown)
                            horario = countdown.replace("/", "").trim();
                    }
                }
                ...
                const parseOrders = (selector) => {
                    ...
                    document.querySelectorAll(`${selector} li.row`).forEach((row) => {
                        const preco = row.querySelector("span.price")?.textContent?.trim() || "";
                        const quantidade = row.querySelector("span.amount")?.textContent?.trim() || "";
                        const acumulado = row.querySelector("span.sum")?.textContent?.trim() || "";
        ...
        "binance.com": async (page) => {
            try {
                ...
                try {
                    const cookieBanner = await page.waitForSelector("#onetrust-banner-sdk", {
                        timeout: 8000,
                    });
                    if (cookieBanner) {
                        const acceptButton = await cookieBanner.$("#onetrust-accept-btn-handler");
                        if (acceptButton)
                ...
                return page.evaluate(() => {
                    ...
                    const fundingBlock = Array.from(document.querySelectorAll("div.ticker-market-list div")).find((el) => el.textContent?.includes("Funding"));
                    if (fundingBlock) {
                    ...
                    const parseOrders = (selector) => {
                        ...
                        document
                            ...
                            .forEach((row) => {
                            const parts = row.querySelectorAll(".row-content .text, .row-content .ask-light, .row-content .bid-light");
                            if (parts.length >= 3) {
            ...
            return page.evaluate(() => {
                ...
                preco =
                    document.querySelector(".mark-price")?.textContent?.trim() ||
                        ...
                        null;
                // --- Funding Rate + Countdown ---
                const blocks = Array.from(document.querySelectorAll("div.quote-block"));
                blocks.forEach((block) => {
                    const title = block.querySelector("div.title")?.textContent?.trim() || "";
                    const value = block.querySelector("div.rtl-text.value")?.textContent?.trim() ||
                        "";
                ...
                const parseOrders = (selector) => {
                    ...
                    document
                        ...
                        .forEach((row) => {
                        const price = row.querySelector(".depth-price")?.textContent?.trim() || "";
                        const vols = Array.from(row.querySelectorAll(".depth-volume")).map((v) => v.textContent?.trim() || "");
                        if (vols.length >= 2) {
                            rows.push({
                                preco: price,
                                quantidade: vols[0],
        ...
        "derivatives.bitmart.com": async (page) => {
            ...
            return page.evaluate(() => {
                ...
                const blocks = Array.from(document.querySelectorAll("div._8yAOs7 div.yTl7gX"));
                blocks.forEach((block) => {
                    const label = block.querySelector("span.BSAKJA")?.textContent?.trim() || "";
                    const values = Array.from(block.querySelectorAll("em.xFoJDh")).map((v) => v.textContent?.trim() || "");
                ...
                const parseOrders = (selector, type) => {
                    const orders = [];
                    let acumulado = 0;
                    const rows = document.querySelectorAll(`${selector} .K7gXn8[buy-or-sell="${type}"]`);
                    rows.forEach((row) => {
                        const preco = row.querySelector("span.DFlhh1")?.textContent?.trim() || "";
                        const quantidadeStr = row.querySelectorAll("span.YFFhx2")[0]?.textContent?.trim() ||
                            "0";
                        const quantidade = parseFloat(quantidadeStr.replace(/,/g, "")) || 0;
                        acumulado += quantidade;
                        orders.push({
                            preco,
                            quantidade: quantidadeStr,
        ...
        "blofin.com": async (page) => {
            ...
            return page.evaluate(() => {
                const precoEl = document.querySelector("div.OrderBook_last-price__oxD8C");
                const preco = precoEl?.textContent?.trim() || null;
                // Funding / Countdown
                let taxa = null;
                let horario = null;
                const fundingLi = Array.from(document.querySelectorAll("li")).find((li) => li.textContent?.includes("Funding Rate / Countdown"));
                if (fundingLi) {
                    const text = fundingLi.querySelector("span:last-child")?.textContent || "";
                    const percMatch = text.match(/[-+]?\d+(\.\d+)?%/);
                    const timeMatch = text.match(/\d{2}:\d{2}:\d{2}/);
                    taxa = percMatch ? percMatch[0] : null;
                    horario = timeMatch ? timeMatch[0] : null;
                ...
                const processOrders = (selector) => {
                    ...
                    return rows.map((row) => {
                        ...
                        const quantidade = spans[1]?.textContent?.trim().replace("K", "000") || "";
                        acumulado += parseFloat(quantidade.replace(/,/g, "")) || 0;
                        return { preco, quantidade, acumulado: acumulado.toString() };
                    });
                };
            ...
            return page.evaluate(() => {
                ...
                preco =
                    txt(document.querySelector(".last-price-wrap .last-price span")) ||
                        txt(document.querySelector(".sign-price-wrap .c-common-sub-num span"));
                // -------------------------
                // Funding (taxa + horário)
                // -------------------------
                const fundingFlex = document.querySelector(".via-ticker-item.flex");
                if (fundingFlex) {
                    const leftRate = fundingFlex.querySelector(".ticker-item.pe-2.text-end .text span.rtl\\:unicode-plain") ||
                        fundingFlex.querySelector(".ticker-item.pe-2.text-end .text span");
                    taxa = txt(leftRate);
                    const rightCountdown = fundingFlex.querySelector(".ticker-item.ps-2.text-start .text span");
                    horario = txt(rightCountdown);
                }
                // -------------------------
                // Orderbook (asks e bids)
                // -------------------------
                const asks = Array.from(document.querySelectorAll(".list-item.sell")).map((el) => {
                    const preco = txt(el.querySelector(".td-item:nth-child(1) span span")) || "0";
                    const quantidade = txt(el.querySelector(".td-item:nth-child(2) span span")) || "0";
                    const acumulado = txt(el.querySelector(".td-item:nth-child(3) span span")) || "0";
                    return { preco, quantidade, acumulado };
                });
                const bids = Array.from(document.querySelectorAll(".list-item.buy")).map((el) => {
                    const preco = txt(el.querySelector(".td-item:nth-child(1) span span")) || "0";
                    const quantidade = txt(el.querySelector(".td-item:nth-child(2) span span")) || "0";
                    const acumulado = txt(el.querySelector(".td-item:nth-child(3) span span")) || "0";
                    return { preco, quantidade, acumulado };
        ...
        "app.hyperliquid.xyz": async (page) => {
            ...
            return page.evaluate(() => {
                ...
                // -------------------------
                const items = Array.from(document.querySelectorAll("div.futures_header_infolist ul.item"));
                items.forEach((item) => {
                    const label = item.querySelector("li.label")?.textContent?.trim();
                    const value = item.querySelector("li.value")?.textContent?.trim();
                    if (!label)
                    ...
                    switch (true) {
                        ...
                        case label.includes("Funding"):
                            const fullText = item.querySelector("li.value")?.textContent || "";
                ...
                // Orderbook (asks e bids)
                // -------------------------
                const rows = Array.from(document.querySelectorAll("div[style*='display: grid']"));
                const asks = [];
                const bids = [];
                rows.forEach((row) => {
                    const bg = row
                    ...
                    const quantidadeVal = txt(row.querySelector("div:nth-child(2) > div"));
                    const acumuladoVal = txt(row.querySelector("div:nth-child(3) > div"));
        ...
        for (const cell of coin.fundingCells) {
            if (typeof cell === "object" && "link" in cell) {
                ...
                try {
                    ...
                    await detailPage.setUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.188 Safari/537.36");
                    await detailPage.evaluateOnNewDocument(() => {
                        Object.defineProperty(navigator, "webdriver", { get: () => false });
                    });
                    console.log(`Abrindo link: ${cell.link}`);
                    await detailPage.goto(cell.link, {
                        waitUntil: "domcontentloaded",
                        timeout: 40000,
                    });
                    await detailPage.evaluate(() => window.scrollBy(0, window.innerHeight));
                    await new Promise((resolve) => setTimeout(resolve, 2000));
                    const host = new URL(cell.link).hostname.replace("www.", "");
                    const extractor = Object.entries(selectors).find(([domain]) => host.includes(domain))?.[1];
                    if (!extractor) {
                        console.log(`${coin.coinName} sem configuração de seletores para ${cell.link}`);
                    }
                    else {
                        const result = await extractor(detailPage);
                        console.log(`${coin.coinName} =>`, result);
                    }
                }
                catch (err) {
                    console.log(`${coin.coinName} erro ao abrir link, pulando...`);
                }
                finally {
                    if (detailPage)
